...
def checkFile(outfile):
    ...
    while os.path.isfile(outfile) is True:
        overwrite = \
            input('[!] Output data file "%s" already exists, overwrite? [y/n] '
                       % outfile)
        if overwrite in ('y', 'Y'):
            return outfile
        elif overwrite in ('n', 'N'):
            outfile = input('[?] Enter new output data file name: ')
            if outfile == '':
                outfile = 'temp.h'
    return outfile
...
def saveDAT(
    ...
    for (index, node) in enumerate(nodes):
        data += '  {(long)(' + str(round(float(node[0]), 5)
                                   * param_scale) + '*PRES), ' \
            + '(long)(' + str(round(float(node[1]), 5) * param_scale) \
            + '*PRES), ' + '(long)(' + str(round(float(node[2]), 5)
                * param_scale) + '*PRES)},\n'
    data += '''};
...
'''
    ...
    for (index, normal) in enumerate(normals):
        data += '  {(long)(' + str(round(float(normal[0]), 5)) \
            + '*PRES), ' + '(long)(' + str(round(float(normal[1]), 5)) \
            + '*PRES), ' + '(long)(' + str(round(float(normal[2]), 5)) \
            + '*PRES)},\n'
    data += '''};
...
def parseSTL(infile, outfile):
    ...
    if check != 0:
        print('[!] Error with STL file, each triangle should be made of 3 vertices (missing %d vertex)' \
            % (3 - check))
        sys.exit(2)
    ...
    [unique_nodes.append(item) for item in nodes if item
     not in unique_nodes]
    ...
    if param_verbose is True:
        for node in unique_nodes:
            ...
            printVerbose('    ' + str(node[0]) + ', ' + str(node[1])
                         + ', ' + str(node[2]) + '\n')
    ...
    if param_normals is True:
        ...
        for line in stl:
            ...
            token = line.split()
            if len(token) == 5 and token[0] == 'facet' and token[1] \
                == 'normal':
...
def main(argv):
    ...
    try:
        (opts, args) = getopt.getopt(sys.argv[1:], 'i:o:s:nhyv', [
            'help',
            'yes',
            'normals',
            'scale=',
